// -----JS CODE-----
// BodyTrackingController.js
// Version: 0.0.1
// Event: Initialized
// Description: The primary script that drives the body template.
...
// @ui {"widget":"group_start", "label":"Left Shoulder Customize", "showIf": "isLeftShoulderTracking"}
...
// @ui {"widget":"group_start", "label":"Right Shoulder Customize", "showIf": "isRightShoulderTracking"}
...
function initialize()
{
    ...
    {
        print( "BodyTrackingController, ERROR: Please assign Orthographic Camera scene object under the advanced checkbox" );
        return false;
    }

    if( !script.spriteMaterial )
    {
        print( "BodyTrackingController, ERROR: Please assign sprite_material from resources panel to the script under the advanced checkbox" );
        return false;
    }

    if( !script.rightShoulderTracker )
    {
        print( "BodyTrackingController, ERROR: Please make sure Right Shoulder Tracker object exist and assign it to the script under advanced checkbox" );
        return false;
    }

    if( !script.leftShoulderTracker )
    {
        print( "BodyTrackingController, ERROR: Please make sure Left Shoulder Tracker object exist and assign it to the script under advanced checkbox" );
        return false;
    }

    if( !script.centerTracker )
    {
        print( "BodyTrackingController, ERROR: Please make sure Center Tracker object exist and assign it to the script under advanced checkbox" );
        return false;
    }

    if( !script.hintControllerScript )
    {
        print( "BodyTrackingController, WARNING: Please assign Object Tracking Hint Controller object to the script under the advanced checkbox" );
    }
...
function assignTexture( option )
{
    ...
    if( option.isEnabled )
    {
        for( var i = 0; i < option.trackerObject.getSceneObject().getChildrenCount(); i++ )
        {
            ...
            {
                print("BodyTrackingController, ERROR: Please assign texture to the texture input");
            }
        }
    }
}
